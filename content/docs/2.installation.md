...
There is no auto-provisioning: a local user must already exist with its `username` set to the value the identity provider sends in the `email` claim. The first successful SSO login for that email links the two accounts to that specific provider; every login after that is matched directly, even if the username is later changed. See the [Admin OIDC Providers API reference](/docs/internal-api/admin/oidc-providers/list) to manage providers programmatically instead of through the console.
